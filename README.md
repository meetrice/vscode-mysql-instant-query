...
Mysql Instant Query 是一款以 SQL 为核心的专业级数据库查询与开发工具，支持 MySQL、PostgreSQL、DuckDB、SQLite 等主流数据库，可从数据表快速生成 SQL、在编辑器中补全与多选字段、即时执行并可视化呈现查询结果，同时提供列名过滤、记录过滤、专业 ERD 实体关系图、自定义图形连接以及 Cursor 文件协议外部链接定位能力，帮助开发者高效完成数据库查询、分析与开发工作。
...
- **SELECT 字段补全与多选** - 在 `SELECT ... FROM 表` 语句中支持字段名补全与「多选字段」QuickPick，勾选后自动生成 SELECT 列表
- **字段筛选值与 WHERE 生成** - 可为字段设置筛选条件，确认后自动生成 `WHERE` 子句；字符串默认 `LIKE '%值%'`，数值/布尔类型按列类型生成合适比较表达式
- **查询结果可视化** - SQL 执行结果以结构化表格呈现，支持大结果集浏览、列宽调整和结果面板交互
- **结果面板复用** - 同一数据表的查询结果复用同一结果面板，减少重复打开标签
- **智能 LIMIT 策略** - 可根据表数据规模配置自动 LIMIT，兼顾查询性能与结果完整性

### SQL 字段补全交互
- **多选 QuickPick** - 在 SELECT 与 FROM 之间触发字段多选，支持全选、取消全选与关闭
- **注释模糊搜索** - 搜索框支持按字段名、类型、注释模糊匹配，快速定位目标字段
- **筛选值输入** - 双击字段或点击「设置筛选值」输入筛选条件；已设置筛选值的字段自动保持勾选
...
- **图形交互增强** - 提供缩放、缩略图、居中、撤销、组合/打散、框选、右键拖动画布等交互能力
- **画布标注与图形** - 支持注释便签、文本标签、直线/曲线/箭头线、矩形/圆形及多种矢量图标，样式可配置并随 ERD 持久化
- **表节点样式** - 支持表边框颜色、背景色、透明度等样式配置
- **侧边栏新建 ERD** - 资源树中可直接创建空白 ERD 画布
...
- **设置页本地化** - 设置页正确显示本地化扩展名称
- **侧边栏布局** - 优化查询树与代码片段面板的展示顺序

## 使用场景

- **日常数据查询** - 从表结构快速生成 SQL，使用字段补全与多选快速编写 SELECT，执行后直接在结果界面分析数据
...
在 SQL 编辑器中编写 `SELECT ... FROM 表名` 且光标位于 SELECT 与 FROM 之间时：

1. 按 `Ctrl+Space`（macOS：`Cmd+Space`）触发补全，或选择「多选字段…」
2. 在 QuickPick 中勾选需要的字段；搜索框可按字段名、类型、注释模糊匹配
3. 双击字段或点击「设置筛选值」为字段输入筛选条件（可选）
4. 按 Enter 确认，自动生成完整 SELECT 语句；有筛选值时一并生成 WHERE 条件

QuickPick 工具栏支持全选、取消全选、设置筛选值与关闭。
...
#### ERD 画布与工具
- 侧边栏新增「新建 ERD」，可快速创建空白画布
- 工具栏新增「图形」菜单：直线、箭头线、曲线、圆角矩形、圆形及头像/云/数据库等图标
- 线型图形支持按下左键定起点、拖动预览、松开定终点；矩形/圆形支持拖拽框选绘制
- 选中具体图形后，工具栏「图形」按钮同步显示对应名称与图标
- 画布空白处右键单击切回「选择」工具；右键拖拽可平移画布
- 支持画布撤销（Undo）、元素组合/打散、统一层级管理
- 支持可编辑文本标签（字体、字号、颜色等）与注释便签；注释输入区仅保留外边框
- 表节点支持边框色、背景色、透明度等样式；优化表头布局、菜单层级与表头高度
- 矢量图形与标注数据随 ERD 文件保存/打开
...
- SQL 字段补全与多选、按字段筛选值生成 WHERE
- 查询结果按数据表复用面板；设置页与侧边栏布局优化
